...
public class ExcelAdapter implements ActionListener {
	...
	public ExcelAdapter(JTable myJTable, boolean enablePaste) {
		jTable1 = myJTable;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().compareTo("Copy") == 0) {
			...
			// numrows = columnModel.getColumnCount();
			// numrows = jTable1.getSelectedRowCount();
			
			// See http://java.sun.com/developer/JDCTechTips/2003/tt1021.html
			for( int k = 0;k<numcols;k++)
			{
				TableColumn column = columnModel.getColumn( k);
				sbf.append( column.getHeaderValue() );
				sbf.append( k < numcols-1 ? "\t" : "\n" );
			}
			
			for (int i = 0; i < numrows; i++) {
				for (int j = 0; j < numcols; j++) {
					
					TableColumn y = columnModel.getColumn(j);
					
					Object theValue = jTable1.getValueAt(rowsselected[i],
						j);
					...
					{
						sbf.append( ((IExcelFriendly)theValue).asExcelFriendlyString() );
					}
					else
					{
						sbf.append(theValue);
		...
		if (e.getActionCommand().compareTo("Paste") == 0 && enablePaste) {
			...
			try {
				...
				for (int i = 0; st1.hasMoreTokens(); i++) {
					...
					for (int j = 0; st2.hasMoreTokens(); j++) {
						...
						// value+"at row="+startRow+i+"column="+startCol+j);
					}
				}
			} catch (Exception ex) {
				ex.printStackTrace();
